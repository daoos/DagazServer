...
[-9999, -9999, -9999, -9999, // piecePawn
   200,   400,   400,   200,
   100,   200,   200,   100,
    50,   100,   100,    50,
     0,     0,     0,     0
],
[-9999, -9999, -9999, -9999, // pieceKnight
 -9999, -9999, -9999, -9999,
  -500,   100,   100,  -500,
  -300,   300,   300,  -300,
  -100,    50,    50,  -100
...
[-9999, -9999, -9999, -9999, // pieceLance
    10,    50,    50,    10,
    50,   100,   100,    50,
   100,   200,   200,   100,
   100,   200,   200,   100
...
[  100,   100,   100,   100, // pieceRook
   100,   200,   200,   100,
   150,   300,   300,   150,
   100,   300,   300,   100,
    50,   100,   100,    50
],
[   50,   100,   100,    50, // pieceKing
    10,    50,    50,    10,
     0,    10,    10,     0,
    10,    50,    50,    10,
    50,   100,   100,    50
...
function GetSlot() {
    var ix = Dagaz.AI.g_toMove ? [18, 14, 10, 6, 2, 0, 4, 8, 12, 16] : [0, 4, 8, 12, 16, 18, 14, 10, 6, 2];
    for (var i = 0; i < RESERVE_SIZE; i++) {
...
Dagaz.AI.MakeMove = function(move) {
    ...
    if (flags & moveflagPromotion) {
        ...
        if ((piece & Dagaz.AI.TYPE_MASK) == pieceSilver) newPiece |= pieceLance;
           else if ((piece & Dagaz.AI.TYPE_MASK) == pieceLance)  newPiece |= pieceSilver;
           else if ((piece & Dagaz.AI.TYPE_MASK) == piecePawn)   newPiece |= pieceKnight;
           else if ((piece & Dagaz.AI.TYPE_MASK) == pieceKnight) newPiece |= piecePawn;
           else if ((piece & Dagaz.AI.TYPE_MASK) == pieceTokin)  newPiece |= pieceBishop;
           else if ((piece & Dagaz.AI.TYPE_MASK) == pieceBishop) newPiece |= pieceTokin;
           else if ((piece & Dagaz.AI.TYPE_MASK) == pieceGold)   newPiece |= pieceRook;
...
Dagaz.AI.UnmakeMove = function(move) {
    ...
    if (flags & moveflagPromotion) {
        ...
        if ((Dagaz.AI.g_board[to] & Dagaz.AI.TYPE_MASK) == pieceSilver) piece |= pieceLance;
           else if ((Dagaz.AI.g_board[to] & Dagaz.AI.TYPE_MASK) == pieceLance)  piece |= pieceSilver;
           else if ((Dagaz.AI.g_board[to] & Dagaz.AI.TYPE_MASK) == piecePawn)   piece |= pieceKnight;
           else if ((Dagaz.AI.g_board[to] & Dagaz.AI.TYPE_MASK) == pieceKnight) piece |= piecePawn;
           else if ((Dagaz.AI.g_board[to] & Dagaz.AI.TYPE_MASK) == pieceTokin)  piece |= pieceBishop;
           else if ((Dagaz.AI.g_board[to] & Dagaz.AI.TYPE_MASK) == pieceBishop) piece |= pieceTokin;
           else if ((Dagaz.AI.g_board[to] & Dagaz.AI.TYPE_MASK) == pieceGold)   piece |= pieceRook;
...
Dagaz.AI.GenerateAllMoves = function(moveStack) {
	...
	to = from + 17; while (Dagaz.AI.g_board[to] == 0) { 
             moveStack[moveStack.length] = GenerateMove(from, to, 0); to += 17; 
        }
	to = from + 15; while (Dagaz.AI.g_board[to] == 0) { 
             moveStack[moveStack.length] = GenerateMove(from, to, 0); to += 15; 
        }
	to = from - 17; while (Dagaz.AI.g_board[to] == 0) { 
             moveStack[moveStack.length] = GenerateMove(from, to, 0); to -= 17; 
        }
...
Dagaz.AI.GenerateCaptureMoves = function(moveStack) {
	...
	to = from; do { to += 1; } while (Dagaz.AI.g_board[to] == 0); 
        if (Dagaz.AI.g_board[to] & enemy) {
            moveStack[moveStack.length] = GenerateMove(from, to, moveflagPromotion);
        }
	to = from; do { to -= 16; } while (Dagaz.AI.g_board[to] == 0); 
