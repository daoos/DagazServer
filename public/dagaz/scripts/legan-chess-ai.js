...
Dagaz.AI.g_flags = moveflagPromotion | moveflagPromoteKnight | moveflagPromoteQueen | moveflagPromoteBishop;
...
function IsSquareAttackable(target, color) {
	...
	if (Dagaz.AI.g_board[target - (inc * 16)] == pawn) return true;
...
Dagaz.AI.GenerateAllMoves = function(moveStack) {
    ...
    var color = Dagaz.AI.g_toMove & Dagaz.AI.colorWhite;
...
function MovePawnTo(moveStack, start, square) {
    if (_.indexOf(PROMOTION, square) >= 0) {
...
Dagaz.AI.See = function(move) {
    ...
    if (((Dagaz.AI.g_board[to + inc] & Dagaz.AI.PIECE_MASK) == (piecePawn | them)) ||
        ((Dagaz.AI.g_board[to + (inc * 16)] & Dagaz.AI.PIECE_MASK) == (piecePawn | them))) {
        return false;
    }
    ...
    if (((Dagaz.AI.g_board[to - inc] & Dagaz.AI.PIECE_MASK) == (piecePawn | us)) ||
        ((Dagaz.AI.g_board[to - (inc * 16)] & Dagaz.AI.PIECE_MASK) == (piecePawn | us))) {
        Dagaz.AI.g_board[from] = fromPiece;
        return true;
    }
