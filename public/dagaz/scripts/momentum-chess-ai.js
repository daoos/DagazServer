...
function getDir(from, to) {
    var fX = from & 0x0F; var tX = to & 0x0F;
    var fY = from & 0xF0; var tY = to & 0xF0;
    var dx = tX - fX; var dy = tY - fY;
    if (dy < 0) {
        if (dx < 0) {
            return -17;
        } else if (dx > 0) {
...
function toEnd(pos, dir) {
    var to = pos; var p = to + dir;
    while (Dagaz.AI.g_board[p] == pieceEmpty) {
        to = p;
        p += dir;
    }
    return to;
...
Dagaz.AI.MakeMove = function(move) {
    ...
    if (flags & moveflagPromotion) {
        // TODO: Превращения работают на конечной позиции, даже в отсутсвии флага
...
function IsSquareAttackableFrom(target, from) {
    ...
    if (_.isUndefined(g_vectorDelta[index])) {
        console.log(FormatSquare(from) + ' --> ' + FormatSquare(target));
    }
