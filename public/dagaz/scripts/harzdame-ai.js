...
[   0,    0,   10,   10,   10,   10,   10,   10, // pieceMan
    0,    0,    0,    0,    0,    0,    0,   10,
    0,    0,    0,    0,    0,    0,    0,   10,
    0,    0,    0,    0,    0,    0,    0,   10, 
    0,    0,    0,    0,    0,    0,    0,   10, 
...
function GenerateQuietStep(moves, from, to, isMan) {
    var me = (Dagaz.AI.g_toMove & Dagaz.AI.colorWhite) >> Dagaz.AI.TYPE_SIZE;
    var flags = 0;
    if (isMan) {
        if (Dagaz.AI.pieceAdj[pieceMan][me == 0 ? Dagaz.AI.flipTable[to] : to]) {
    ...
    for (var from = 0; from < 256; from++) {
         if (Dagaz.AI.g_board[from] & me) {
             ...
             if (piece == pieceMan) {
                 ...
                 to = from - (-16 * inc); if (Dagaz.AI.g_board[to] == 0) mob++;
                 result += 10 * mob;
             }
             if (piece == pieceKing) {
                 mob = -2;
