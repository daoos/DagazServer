...
//  this.stage = 3;
    ...
    this.nextMove = function () {
        if (++this.atMove == this.moveCount) {
            ...
            if (this.stage == 2) {
                this.moves = Dagaz.AI.GenerateCaptureMoves(this.moves);
                ...
                for (var i = this.atMove; i < this.moveCount; i++) {
                    ...
                    var step = this.moves[i][0];
                    var captured = Dagaz.AI.g_board[(step >> 8) & 0xFF] & Dagaz.AI.TYPE_MASK;
                    var pieceType = Dagaz.AI.g_board[step & 0xFF] & Dagaz.AI.TYPE_MASK;
                    var target = (step >> 16) & 0xFF;
                    if (target) {
                        captured = Dagaz.AI.g_board[target] & Dagaz.AI.TYPE_MASK;
                    }
            ...
            if (this.stage == 5) {
                this.moves = Dagaz.AI.GenerateAllMoves(this.moves);
                this.moveCount = this.moves.length;
                // Move ordering
...
function AllCutNode(ply, depth, beta, allowNull) {
    if (ply <= 0) {
        return QSearch(beta - 1, beta, 0, depth);
    }
    ...
    if (!Dagaz.AI.g_inCheck &&
        ...
        if (hashMove == null &&
            ...
            if (Dagaz.AI.g_baseEval < beta - razorMargin) {
                ...
                var v = QSearch(razorBeta - 1, razorBeta, 0, depth);
                if (v < razorBeta)
                    return v;
            }
        }
    ...
    for (;;) {
        ...
        if ((currentMove == 0) || (currentMove.length == 0)) {
            break;
        }
        var currentStep = currentMove[0];
        ...
        if (value > realEval) {
            if (value >= beta) {
		var histTo = (currentStep >> 8) & 0xFF;
		if (Dagaz.AI.g_board[histTo] == 0) {
                    if (Dagaz.AI.USE_HIST_TABLE) {
                        var histPiece = Dagaz.AI.g_board[currentStep & 0xFF] & Dagaz.AI.PIECE_MASK;
		    ...
		    if (g_killers[depth][0] != currentStep) {
		        g_killers[depth][1] = g_killers[depth][0];
		        g_killers[depth][0] = currentStep;
		    }
		}
...
Dagaz.AI.ScoreMove = function(move) {
    ...
    var step = move[0];
    var moveTo = (step >> 8) & 0xFF;
    var captured = Dagaz.AI.g_board[moveTo] & Dagaz.AI.TYPE_MASK;
    var piece = Dagaz.AI.g_board[step & 0xFF];
    var target = (step >> 16) & 0xFF;
    if (target) {
        captured = Dagaz.AI.g_board[target] & Dagaz.AI.TYPE_MASK;
    }
